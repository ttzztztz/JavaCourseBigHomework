...
const App: React.FC = () => {
  ...
  const sortListAsc = () => {
    setLists([...lists.sort(($1, $2) => ($1.name < $2.name ? 1 : -1))]);
  };
  const sortListDesc = () => {
    setLists([...lists.sort(($1, $2) => ($1.name < $2.name ? -1 : 1))]);
  };
  ...
  return (
    <div className="App">
      <BrowserRouter>
        <div className="list">
          <TaskList all={lists} sortAsc={sortListAsc} sortDesc={sortListDesc} />
          <div className="task-container">
            <Switch>
              <Redirect from="/" to="/list/all/1" exact />
              <Route
                path="/list/:lid/:page"
                component={() => <ListView lists={lists} />}
              />
              <Route path="/item/:tid" component={ItemView} />
              <Route path="/create/list" component={EditList} />
              <Route
                path="/create/task"
                component={() => <EditTask lists={lists} />}
              />
